...
public class TrustDeclarationAsIngredient {
    ...
    void testActionsAssertion() throws Exception {
        ...
        String assetFileUrl = initialAssetFileUrl.replace("sample", "s12");

        JumbfBox trustRecord = constructTrustRecordForScenario(initialAssetFileUrl);

        String targetFileUrl = assetFileUrl.replace(".jpeg", "-trust-declaration-as-ingredient-2ed.jpeg");
        jpegCodestreamGenerator.generateJumbfMetadataToFile(List.of(trustRecord), initialAssetFileUrl,
                targetFileUrl);
